...
# Script: send_json_to_server2.sh
...
# File Names
file_name_pattern="filesystem*"
file_name_log="send_json_to_server2_${current_date_log}.log"

# File Directories
tmp_directory="/tmp"
file_destination="/opt/filesystem"
...
{
	# Check if a latest filesystem JSON file can be found under /tmp/
...
function TransferToRemoteServer()
{
	...
	# Check if '/opt/filesystem' exists on Server 2. If yes, change directory owner to root. If not, create one.
	...
	if ssh "$username@$ip_address" "ls $file_destination >/dev/null 2>&1" ; then
		...
		if [ $? -ne 0 ]; then
			PrintMessage "There's a possiblity that the password given for Server 02 was incorrect or not valid! Terminating the script..." 1
			exit
		fi
	else
